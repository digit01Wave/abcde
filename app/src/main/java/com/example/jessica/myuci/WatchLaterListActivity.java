...
public class WatchLaterListActivity extends AppCompatActivity {
    ...
    public void syncWatchLaterSQLiteMySQLDB(){
        ...
        if(controller.dbWatchLaterSyncCount() != 0){
            prgDialog.show();
            params.put(ServerEntry.JSON_UPDATE_TITLE, controller.composeJSONfromWatchLaterSQLite());
                    ...
                    try {
                        ...
                        for(int i=0; i<arr.length();i++){
                            ...
                            controller.updateWatchLaterSyncStatus(obj.getString(WLEntry.COLUMN_NAME_USER_ID),
                                    obj.getString(WLEntry.COLUMN_NAME_EVENT_ID),
                                    obj.getString(ServerEntry.UPDATE_ACTION_TITLE),
                                    obj.getString(WLEntry.COLUMN_NAME_UPDATE_STATUS));
                        }
